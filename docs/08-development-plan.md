...
- `awaiting_manual`：自动化全门禁已通过，`04-test-report.md` 完整，等待人工验证。
- `passed`：`05-exit-report.md` 和所需人工证据完整，依赖迭代可进入 `ready`。
...
1. 阅读 `AGENTS.md`、状态、三份入口文档和引用的产品规格。
2. 检查工作树，识别用户已有改动。
3. 将迭代置为 `in_progress`，按测试用例矩阵准备能证明需求的聚焦测试与夹具。
4. 以最小完整能力实施范围内工作；不做后续迭代和假 UI。
5. 开发期间运行聚焦测试；在迭代退出前运行 `pnpm check` 以及适用的 E2E、性能和安全套件。
6. 同步接口、ADR、兼容矩阵和迭代文档；自动化证据写入 `04-test-report.md`。
7. 有人工门禁时设置 `awaiting_manual` 并停止；人工证据完成后写入 `05-exit-report.md`，再设置 `passed`。
...
迭代进入 `ready` 前，三份入口文档必须具备需求 ID、依赖、非目标、设计决策、预期文件、自动测试、人工验证、完成条件和失败回退。缺少任一项时先补全迭代文档，不直接实现。

完成条件：所有明确验收有直接证据且测试未被弱化；`pnpm check` 和适用的 E2E、性能、安全套件通过；无新增未解释 warning、临时开关、假按钮或跳过测试；文档、接口和兼容矩阵已同步；人工门禁完成并记录后才可 `passed`。
